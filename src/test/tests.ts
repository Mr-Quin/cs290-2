import { describe, expect, test } from './runner'
import type { CheckRecursionResult } from '../utils/checkRecursion'
...
const generateArbitraryObject = (depth: number): any => {
  if (depth <= 0) {
    return { [generateRandomString(10)]: generateRandomString(10) }
  }

  return { [generateRandomString(10)]: generateArbitraryObject(depth - 1) }
}
...
export const runTests = (
  ...
  describe(name, () => {
    ...
    test('Arbitrarily nested objects 1', () => {
      const obj1 = { a: { b: { c: { f: { g: 5 }, d: { e: 1 } } } } }
      const obj2 = { a: { b: { c: { d: { e: 1 }, f: { g: 5 } } } } }
      expect(deepEqual(obj1, obj2)).toBe(true)
    })

    test('Arbitrarily nested objects 2', () => {
      const obj1 = { a: { b: { c: { f: { g: 5 }, d: { e: 1 } } } } }
      const obj2 = { a: { b: { c: { d: { e: 1 }, f: { g: 6 } } } } }
      expect(deepEqual(obj1, obj2)).toBe(false)
    })

    test('Random objects', () => {
      const obj1 = generateArbitraryObject(10)
      const obj2 = cloneObject(obj1)
      expect(deepEqual(obj1, obj2)).toBe(true)
    })

    test('Random objects 2', () => {
      const obj1 = generateArbitraryObject(10)
      const obj2 = generateArbitraryObject(10)
      expect(deepEqual(obj1, obj2)).toBe(false)
    })

    test('Contains recursion', () => {
      expect(context.recursiveFns.size > 0).toBe(true)
    })
  })
}
